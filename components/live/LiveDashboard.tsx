                ...
                {inPlayMatches.map((match) => (
                  <div key={`live-${match.id}`} className="bg-red-900/20 p-2 md:p-4 rounded-lg border border-red-500/40 flex justify-between items-center transition-colors">
                    <div className="flex flex-col items-end w-[35%] md:w-[40%]">
                      <span className="text-xs md:text-body-md md:font-body-md text-on-surface font-semibold truncate max-w-full">{match.homeLabel || (match.homeTeam?.name ? match.homeTeam.name : `Match ${match.matchNumber || match.id}`)}</span>
                    ...
                    <div className="flex flex-col items-start w-[35%] md:w-[40%]">
                      <span className="text-xs md:text-body-md md:font-body-md font-semibold text-primary-container truncate max-w-full">{match.awayLabel || (match.awayTeam?.name ? match.awayTeam.name : `Match ${match.matchNumber || match.id}`)}</span>
